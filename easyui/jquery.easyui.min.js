...
if(data.resultValue != null){
	if(data.resultValue.nodes != null){
	data = data.resultValue.nodes;
	}
}
...
var _13z=$.data(_12f,"tree");
if(_13z==undefined){
	return;
}
var opts=_13z.options;
if(opts.lines){
...
var dlg=_2c5(opts);
return dlg;
},confirm:function(_2ca,msg,fn,msg2){
//修改confirm宽度，添加新参数
var opts=typeof _2ca=="object"?_2ca:{title:_2ca,msg:msg,fn:fn,msg2:msg2||""};
var confirmWidth=opts.msg.length>opts.msg2.length?opts.msg.length*18+30:opts.msg2.length*18+30;
confirmWidth=confirmWidth>250?confirmWidth:250;
confirmWidth=confirmWidth<1000?confirmWidth:1000;
$.messager.defaults.width=confirmWidth;
opts=$.extend({},$.messager.defaults,{content:"<div class=\"messager-icon messager-question\"></div>"+"<div class='msg2'>"+opts.msg2+"</div>"+"<div class='msg1'>"+opts.msg+"</div>"+"<div style=\"clear:both;\"/>"},opts);
if(!opts.buttons){
opts.buttons=[{text:opts.ok,onClick:function(){
_2c0(dlg,true);
...
if(opts.required){
/**添加空值处理 add by liangbj 2017-10-21**/
/**if(opts.val(_4d1)==""){**/
var _4d1Value = opts.val(_4d1);
if($.trim(_4d1Value)=="" || ($.trim(_4d1Value)).toUpperCase()=="NULL" ){
/**添加空值处理 add by liangbj  2017-10-21**/
...
var t=form.attr("target"),a=form.attr("action"),enct=form.attr("enctype");
...
_593();
//ECPENCODEFN.ajaxSendEncode("",opts,null,form);
//一次一密组件数据加密
//debugger;
if(window.top.ECPENCODEFN.isEncrypt(a) && a.indexOf("/common/rest/fileupload/upload") > -1 && enct.toLowerCase() == "multipart/form-data"  ){
	//获取表单数据为json对象
	var params=form_json($(form));
	$(form).form('load',params);
	function form_json($form){
		var params=$form.serializeArray();
		var params_obj={};
		for(var i=0;i<params.length;i++){
			params_obj[params[i].name]=window.top.ECPENCODEFN.ecpEncodeInSingleParam(params[i].value);
		}
		return params_obj;
	}
}
...
return xhr;
},data:_597,dataType:"html",cache:false,
/*contentType:false,检修前测试 注释掉*/
contentType:opts.contentType,/*检修前测试放开*/
processData:false,complete:function(res){
...
function _59d(data){
	...
	$.each(data, function(i, val) {  
		if(typeof (val)=="string"){
			data[i]=data[i].replace(/(select|script|update|delete|insert|exec|count|prompt)+/gi, "");//|'|"|=|;|>|<
	        //data.resultValue.items[0][i]=data.resultValue.items[0][i].replace(/((?=[\x21-\x7e]+)[^A-Za-z0-9.])/gi, "");
...
//		if((typeof val)=="string"){
//			data[i]=data[i].replace(/(select|script|update|delete|insert|exec|count|prompt|'|"|=|;|>|<|%|[\\])+/gi, "");
//	        //data[i]=data[i].replace(/((?=[\x21-\x7e]+)[^A-Za-z0-9.])/gi, "");			
//		}
//	});
return jq.each(function(){  
load(this,data);
...
_668._outerWidth(_666.width()+18);
...
if(data.resultValue != null){
	if(data.resultValue.items != null){
		data = {total: data.resultValue.itemCount, rows: data.resultValue.items, dicts:data.dicts,footer: data.resultValue.footer};
	}
}
...
$.each(row, function(i, val) {  
	if(typeof (val)=="string"){
		row[i]=row[i].replace(/(select|script|update|delete|insert|exec|count|prompt|'|"|=|;|>|<)+/gi, "");
		//|[//]
		//row[i]=row[i].replace(/((?=[\x21-\x7e]+)[^A-Za-z0-9.])/gi, "");
	}
});  
...
$.ajax({type:opts.method,url:opts.url,data:_86f,dataType:"json",success:function(data){
	//console.log(data)
	//form 表单提交 防xss攻击
	if(typeof(data.resultValue)=='undefined'){
		$.each(data, function(i, val) {  
			if(typeof (val)=="string"){
				data=data.replace(/(select|script|update|delete|insert|exec|count|prompt|'|"|=|;|>|<)+/gi, "");
				//|[//]
		        //data.resultValue.items[0][i]=data.resultValue.items[0][i].replace(/((?=[\x21-\x7e]+)[^A-Za-z0-9.])/gi, "");
			}
		});  
	}else if(data.resultValue.items.length>0){
		$.each(data.resultValue.items[0], function(i, val) {  
			if(typeof (val)=="string"){
				data.resultValue.items[0][i]=data.resultValue.items[0][i].replace(/(select|script|update|delete|insert|exec|count|prompt)+/gi, "");//|'|"|=|;|>|<
				//|[//]
		        //data.resultValue.items[0][i]=data.resultValue.items[0][i].replace(/((?=[\x21-\x7e]+)[^A-Za-z0-9.])/gi, "");
			}
		}); 
...
_aac.apply(this,arguments);
	...
	//2017年10月8日15:25:12
	//冯东修改combox获取数据方式支持uap返回格式
if(data.resultValue != null){
	if(data.resultValue.items != null){
		data = data.resultValue.items;
	}
}
//end modify
return data;
